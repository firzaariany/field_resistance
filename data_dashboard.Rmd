...
ggplot(wild_yr2000_clean, aes(x = Classe)) +
...
ggplot(wild_yr2000_clean, aes(x = Population)) + 
  geom_bar() +  # barchart
  facet_wrap(~ Classe) +  # faceted by parents and natural pop
  geom_text(stat = "count", aes(label = ..count..), vjust = -0.5) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

```
ValdeLoire = centre valdeloire
Selles_sur_Cher = centre valdeloire
Bonny = centre valdeloire

Ramières = Auvergne-Rhône-Alpes
ValAllier = Auvergne-Rhône-Alpes
St_Jean_de_Maurienne = Auvergne-Rhône-Alpes
Meximieux = Auvergne-Rhône-Alpes
St_Genix_sur_Guiers = Auvergne-Rhône-Alpes

Tarbes = Occitanie region
Nohèdes = Occitanie
Sauveterre = Occitanie
Cauterets = Occitanie

StPrivé = Bourgogne-Franche-Comté
La_Celle_sur_Loire = Bourgogne-Franche-Comté

Sarrazin = Nouvelle-Aquitaine
Gave-Mauleon = Nouvelle-Aquitaine
Baudreix = Nouvelle-Aquitaine
Baliros = Nouvelle-Aquitaine
Assat = Nouvelle-Aquitaine

Pont_de_Cé = Pays de la Loire
Cunault = Pays de la Loire
